  ...
  return (
    ...
    >
      <div
        ...
        className="bg-[#121622] border border-[var(--border-medium)] rounded-2xl max-w-2xl w-full flex flex-col max-h-[85vh] shadow-2xl animate-modal overflow-hidden"
      >
        {/* Modal Header */}
        ...
        {/* Filter Input */}
        <div className="px-6 pt-4 pb-2 border-b border-white/5 bg-[#141824]">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-black/30 border border-white/10 rounded-lg text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
          {filteredSections.map((sec) => (
            <div key={sec.title}>
              <h3 className="text-[11px] font-semibold text-indigo-300 uppercase tracking-wider mb-2.5">
                {sec.title}
              </h3>
              <div className="space-y-1.5">
                {sec.items.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 rounded-lg bg-[#161a28] border border-white/5 hover:border-white/10 transition-colors"
                  >
                    ...
                    <div className="flex items-center gap-1">
                      {item.keys.map((k, kIdx) => (
                        <kbd
                          key={kIdx}
                          className="px-2 py-0.5 rounded bg-black/40 border border-white/15 text-indigo-200 font-mono text-[11px] font-medium shadow-inner"
                        >
                          {k}
                        </kbd>
                      ))}
        ...
        <div className="px-6 py-3 border-t border-[var(--border-subtle)] flex items-center justify-between bg-[var(--bg-glass)] text-xs text-zinc-400">
          ...
          <button
            onClick={() => setShortcutsModalOpen(false)}
            className="px-4 py-1.5 rounded-lg bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-hover)] text-white font-medium shadow-md transition-all text-xs"
          >
            Close
          </button>
        </div>
