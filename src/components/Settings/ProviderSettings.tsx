...
export const ProviderSettings: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 select-none">
      <div className="bg-[var(--bg-dark-elevated)] border border-[var(--border-subtle)] rounded-md max-w-2xl w-full flex flex-col max-h-[85vh] shadow-2xl animate-modal overflow-hidden">
        {/* Modal Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {(['gemini', 'openai', 'anthropic', 'openrouter', 'custom'] as const).map((p) => (
                <button
                  ...
                  className={`p-2.5 rounded-md border text-left transition-all capitalize ${
                    activeProvider === p
                      ? 'bg-sky-600/30 border-sky-500 text-white font-medium'
                      : 'bg-[var(--bg-dark-surface)] border-white/5 text-zinc-400 hover:text-zinc-200 hover:bg-white/5'
                  }`}
                >
          ...
          <div className="p-4 bg-[var(--bg-dark-surface)] border border-white/10 rounded-md space-y-3">
            ...
            <div>
              <label className="block text-[11px] text-zinc-400 mb-1">API Key {activeProvider === 'custom' ? '(optional — only if your local server requires one)' : ''}</label>
              <input
                type="password"
                value={keys[activeProvider] || ''}
                onChange={(e) => handleKeyChange(activeProvider, e.target.value)}
                placeholder={activeProvider === 'custom' ? 'Leave blank for default Ollama setup' : `Paste your ${currentConfig.name} API key...`}
                className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded text-xs text-white focus:outline-none focus:border-sky-500 font-mono"
              />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 bg-black/40 border border-white/10 rounded text-xs text-white focus:outline-none focus:border-sky-500 font-mono"
                />
              </div>
              ...
              <div>
                <label className="block text-[11px] text-zinc-400 mb-1 flex items-center justify-between">
                  ...
                  <span className="text-sky-400 font-mono">{currentConfig.temperature}</span>
                </label>
                <input
                  type="range"
                  min="0.0"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="tvly-..."
                  className="w-full px-3 py-1.5 bg-black/40 border border-white/10 rounded text-xs text-white focus:outline-none focus:border-cyan-500 font-mono"
                />
              </div>
        ...
        <div className="px-6 py-3 border-t border-[var(--border-subtle)] flex items-center justify-between bg-[var(--bg-glass)]">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setSettingsOpen(false)}
              className="px-4 py-1.5 rounded text-zinc-400 hover:text-white transition-colors"
            >
              Done
            </button>
            <button
              onClick={handleSaveKeys}
              className="px-4 py-1.5 rounded bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-hover)] text-[var(--text-on-accent)] font-semibold transition-all cursor-pointer"
            >
              Save Credentials
            </button>
          </div>
