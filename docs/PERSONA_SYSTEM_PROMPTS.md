# Persona System Prompts & LLM Architecture for OmniDoc Studio

---

## 1. System Prompt Architecture: Layered Composition Model

To scale predictably across model tiers—from 70B+/frontier APIs (Claude 3.5 Sonnet, GPT-4o, Gemini 1.5 Pro) down to quantized 8B-class local runtimes (Llama 3.1 8B Instruct, Qwen 2.5 7B/14B)—OmniDoc Studio uses a **three-tier injection pipeline**:
...
2. **Persona Kernel (Layer 2)**: Defines the behavioral archetype. It dictates reasoning style, editing delta tolerances, and output shape.
...
| Persona | Recommended Model Tier | Temperature | Top_P | Default Tool Access | Default Output Length | Clarify vs. Act Policy |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Co-Writer** | Tier 1 (Sonnet / GPT-4o / Qwen-72B) | **0.65** | 0.90 | `read_document`, `propose_insert`, `get_selection` | 200–450 words | **Act Immediately** (offer 2 variants; ask only if genre/audience is completely undefined). |
| **Researcher** | Tier 1 or Local 14B+ | **0.20** | 0.85 | `web_search`, `read_document` | 300–600 words | **Act with Caveats** (synthesize immediately; identify assumptions at the bottom). |
| **Proofreader** | Local 8B / 14B / Tier 1 | **0.10** | 0.80 | `get_selection`, `read_document` | Strict Diff + Clean Pass | **Never Clarify** (correct obvious errors immediately; tag ambiguities as `[Optional]`). |
| **Brainstormer** | Any (8B to Frontier) | **0.85** | 0.95 | None (pure generative) or `web_search` | 250–500 words | **Never Clarify** (generate divergent options immediately; end with decision questions). |
...
### Runtime Dynamic Persona Loader

In OmniDoc Studio's AI service layer (`src/services/aiService.ts`), assemble the prompt dynamically using model capabilities:

```typescript
export function composeSystemPrompt(
  personaId: AIPersona,
  modelId: string,
  runtimeContext?: { docName?: string; format?: string; selectedText?: string }
): string {
  const isSmallModel = modelId.includes('8b') || modelId.includes('7b') || modelId.includes('mini');

  const preamble = STATIC_STUDIO_PREAMBLE;
  const personaCore = PERSONA_PROMPTS[personaId].core;
  const personaExt = isSmallModel ? '' : PERSONA_PROMPTS[personaId].extended;

  let dynamicFrame = '';
  if (runtimeContext?.docName) {
    dynamicFrame = `\n\nACTIVE DOCUMENT:\nName: ${runtimeContext.docName}\nFormat: ${runtimeContext.format || 'text'}`;
  }
  if (runtimeContext?.selectedText) {
    dynamicFrame += `\nSELECTED TARGET TEXT:\n"""\n${runtimeContext.selectedText}\n"""`;
  }

  return `${preamble}\n\n${personaCore}${personaExt ? '\n\n' + personaExt : ''}${dynamicFrame}`;
}
```
